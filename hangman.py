...
def getGuessedWord(secretWord, lettersGuessed):
    ...
    # FILL IN YOUR CODE HERE...
    space = ' '
    game_state = ['_'] * len(secretWord)
    for i in range(len(secretWord)):
        if secretWord[i] in lettersGuessed:
            game_state[i] = secretWord[i]
    return space.join(game_state)
...
def getAvailableLetters(lettersGuessed):
    ...
    for i in range(len(lettersGuessed)):
        if lettersGuessed[i] in alphabet:
            alphabet.pop(lettersGuessed[i])
    return space.join(sorted(alphabet.values()))
...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    while isWordGuessed(secretWord, lettersGuessed) is False and guesses > 0:
        ...
        elif guessedLetter not in lettersGuessed and guessedLetter not in secretWord:
            lettersGuessed.append(guessedLetter)
            guesses -= 1
            print("Oops! That letter is not in my word: {}".format(getGuessedWord(secretWord, lettersGuessed)))
        elif guessedLetter not in lettersGuessed and guessedLetter in secretWord:
            lettersGuessed.append(guessedLetter)
